...
from app.services.ml_layer import ml_block, strip_forbidden
...
async def get_entity_detail(
    ...
    if not node:
        raise HTTPException(status_code=404, detail="Node tidak ditemukan")
    # Aditif: `entity` tetap dikembalikan apa adanya (termasuk
    # `riskScore`, `riskLevel`, dan `triggeredRules` dari rule engine v2), dan
    # blok `ml` berdiri di sampingnya. Tidak satu pun field lama disentuh.
    #
    # `strip_forbidden` hanya membuang properti `gt_*` bila entah bagaimana ada
    # di Neo4j; jalur impor resmi sudah menolaknya lebih dulu. Ground truth yang
    # terbit lewat endpoint ini akan terlihat responden dan membatalkan studi
    # responden yang bergantung pada data ini.
    entity = strip_forbidden(node)
    return {"entity": entity, "ml": ml_block(entity, node_id)}
...
async def get_risk_analysis(
    ...
    # Aditif. `assessment` (berisi `explanation` dan `triggeredRules`),
    # `evidencePath`, `earlyWarnings`, dan `blacklistCandidate` dikembalikan
    # persis seperti sebelumnya. Rule engine tidak disentuh: `analyze_entity`
    # dan `scoring.py` tidak diubah satu baris pun.
    #
    # Keterbatasan yang sengaja tidak ditambal: 15 rule v1 (R-001..R-015)
    # mengandalkan tipe node v1 (`URL`, `Keyword`, `Transaction`, `TrafficEvent`,
    # `BlacklistEntity`) yang tidak ada di kontrak data v2, sehingga
    # `triggeredRules` di sini akan nyaris kosong atas data sintetik v2. Rule
    # engine v2 yang sungguhan adalah milik orang A, dan keluarannya sudah
    # tersaji sebagai `riskScore`/`riskLevel`/`triggeredRules` pada properti
    # node (lihat blok `ruleV2` di bawah dan `integration/import_synthetic.py`).
    payload = result.as_dict()
    node = await repo.get_node(node_type, node_id)
    entity = strip_forbidden(node or {})
    payload["ml"] = ml_block(entity, node_id)
    payload["ruleV2"] = {
        "riskScore": entity.get("riskScore"),
        "riskLevel": entity.get("riskLevel"),
        "triggeredRules": entity.get("triggeredRules", []),
        "source": "rules/ orang A via weak_labels.csv",
    }
    return payload
